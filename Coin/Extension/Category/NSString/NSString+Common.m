...
//        fullPath = [NSString stringWithFormat:@"%@%@", API_BASE_URL, path];
        fullPath = [kAppBaseUrl stringByAppendingPathComponent:path];
...
- (BOOL)containString:(NSString *)string {
    ...
    if (!(VERSION_8_0_LATER)) {
        if (![NSString isEmpty:self] && [self rangeOfString:string].location != NSNotFound) {
            return YES;
        }
    } else {
        if ([esString(self) containsString:string]){
            return YES;
        }
    }
    return NO;
